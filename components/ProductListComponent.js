...
const { products: initialProducts } = require('@/components/Data/data');
...
const ProductListComponent = () => {
  ...
  const handleDelete = (id) => {
    if (confirm('Are you sure you want to delete this product?')) {
      setProducts(products.filter(cat => cat.id !== id));
    }
  };
  ...
  const handleSave = () => {
    ...
    if (editingProduct) {
      setProducts(products.map(cat => 
        cat.id === editingProduct.id ? newProduct : cat
      ));
    } else {
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600">Manage your product products efficiently</p>
        </div>
        ...
        {/* Add/Edit Modal */}
        {showAddModal && (
         <AddProductModal  handleSave={handleSave} closeModal={closeModal} formData={formData} setFormData={setFormData} editingProduct={editingProduct} />
        )}
      </div>
